...
$cache = $modx->getOption('cache', $scriptProperties, 1);

if ($elementClass == 'modSnippet' && empty($cache)) {
    $output = $modx->runSnippet($element, $scriptProperties);
    //https://github.com/bezumkin/pdoTools/issues/33
    $output = str_replace('[![!', '[[!', $output);
} else {
    
    $scriptProperties['mml_cultureKey'] = $modx->getOption('mml_cultureKey', $scriptProperties,
    ...
    */
    ...
    if (empty($cacheHandler))
        $cacheHandler = $modx->getOption('cache_resource_handler', null, $modx->
            getOption(xPDO::OPT_CACHE_HANDLER, null, 'xPDOFileCache'));
    if (!isset($cacheExpires))
        $cacheExpires = (integer)$modx->getOption('cache_resource_expires', null, $modx->
            getOption(xPDO::OPT_CACHE_EXPIRES, null, 0));
    if (empty($cacheElementKey))
        $cacheElementKey = $modx->resource->getCacheKey() . '/' . md5($modx->toJSON($properties) .
            $modx->toJSON($modx->request->getParameters()));
    $cacheOptions = array(
